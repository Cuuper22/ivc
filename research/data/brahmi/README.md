...
This directory holds adversarial Brahmi back-door datasets and their null gates. The back door under attack is a tempting one: if early Brahmi letter shapes descend from Indus signs, then a Brahmi letter's known reading could anchor the value of an Indus sign. A null gate is a pre-registered random control — a candidate match must beat what chance shape evolution produces before it can count as evidence. So far, nothing has.
...
The first gate produced these files:
...
Current result: no Brahmi descent line or phonetic anchor is accepted. The three local `220` source probes fail both family consistency and the random shape-evolution null: their top-1 Brahmi labels are `kaṃ`, `o`, and `ka`, with null <= observed shares `0.576000`, `0.438000`, and `0.656000`. The local `110` source probe's nearest neighbor is `a`, with null <= observed share `0.666000`. In plain terms: random shape evolution matches Brahmi at least as well as the real probes do far too often for these matches to mean anything.

The second gate widened both sides of the comparison and produced these files:
