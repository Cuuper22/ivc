...
ROOT = Path.cwd()
RUN_DATE = "2026-05-29"
PACKET_ID = "directionality_no_overlay_packet_v2c_ocr_coord_fix_controls"

REPORTS = ROOT / "data" / "open_prototype" / "reports"
OUT_DIR = ROOT / "tmp" / "effective_unicity_directionality_blind_packet_v2c"
...
CROP_MANIFEST = REPORTS / "effective_unicity_directionality_blind_packet_v2c_crop_manifest.csv"
BLIND_MANIFEST = REPORTS / "effective_unicity_directionality_blind_packet_v2c_manifest.csv"
ANSWER_KEY = REPORTS / "effective_unicity_directionality_blind_packet_v2c_answer_key.csv"
REVIEW_TEMPLATE = REPORTS / "effective_unicity_directionality_blind_packet_v2c_review_template.csv"
SUMMARY_JSON = REPORTS / "effective_unicity_directionality_blind_packet_v2c_summary.json"
CONTACT_SHEET = OUT_DIR / "directionality_no_overlay_v2c_blind_contact_sheet.png"
...
def mask_ocr_words(
    ...
    for box in scaled_word_boxes(page_info, image.size):
        ...
        y2 = min(crop.height, box[3] - crop_box[1] + 32)
...
def build_route_negative_rows(
    ...
    for cisi in REAL_ROUTE_NEGATIVE_CISIS:
        ...
        spec = {
            ...
            "text": route["text"],
            "source_note": (
                "Forger-recommended routed real negative from the corrected v2 public route probe. Context crop is cut "
                "from raw page, with OCR word boxes parsed under the five-coordinate DjVu fix and masked before blinding; "
                "still requires human leakage QA."
            ),
        }
        rows.append(
            write_crop(
...
def main() -> None:
    ...
    summary = {
        "date": RUN_DATE,
        "packet_id": PACKET_ID,
        "status": "v2c_packet_created_failed_visual_preflight_no_claim_promotion",
        "purpose": (
            "Attempt to repair the v2b real-negative label-leak failure by rebuilding routed controls from a corrected "
            "public-route probe whose DjVu five-coordinate OCR boxes no longer create tall bogus masks. Human visual "
            "preflight still found catalogue/page labels and page-context leakage, so v2c is not reviewer-ready and "
            "promotes no claim."
        ),
        "counts": {
        ...
        "forger_control_plan": {
            ...
            "ocr_coordinate_policy": "DjVu WORD coords with five values are parsed as x1,y1,x2,y2,baseline; the fifth value is ignored instead of being treated as a y-coordinate.",
            "duplicate_hash_policy": "any duplicate blind image hash keeps the packet at preflight failure; v2c is generated to avoid all duplicate target-image hashes",
            "status": "real-control denominator count meets the forger floor, but visual preflight failed because the rows are still page-context crops; the packet is not reviewed and cannot promote anything",
        },
        ...
        "label_masking": {
            ...
            "visual_leakage_blocks_blind_review": True,
            "visual_preflight_decision": "failed_not_reviewer_ready",
            "visual_preflight_failure_mode": "OCR-word masking does not remove all printed catalogue/page labels or adjacent page-context cues.",
        },
        "promotion_thresholds": [
            ...
            "Paired target-view promotion is disabled in v2c because only unique target images are retained; any future paired-view packet must use distinct hashes and identical blind token counts.",
