...
This directory starts Vector 1: reconstructing a diffuse Meluhha bilingual from readable cuneiform attestations plus external Indus-style objects.

It is not a claim ledger and contains no accepted external anchor yet. The current tables are reproducible infrastructure:
...
The next rule is simple: join by date, provenience, object context, and source route first. Do not join by desired reading.

Current expanded inventory: 16 source-side rows from 10 requested source pages, with 18 successful fetches and zero failed fetches on 2026-05-29. The first literal site-overlap join surface has 25 rows across Ur, Girsu, Tello, and Nippur, but the forger rejects site overlap alone as evidence. Accepted external anchors remain zero.

Current hard-anchor test: CDLI current exports find `lu2-sun2-zi-da` in 15 distinct artifacts, but only one has any Meluhha line and only that one has adjacent Meluhha. The name-alone Meluhha detector therefore has a measured false-positive rate of 0.933333. `P212982` remains a real cuneiform-side lead, not an Indus sign-value anchor.

Current external-object phonetic attempt: strict one-sign-per-cuneiform-unit matching against 29 Mesopotamia/Gulf focus rows generated apparent assignments for `me-luh-ha`, `ma2-me-luh-ha`, `lu2-sun2-zi-da`, and `szu-i3-li2-su`, but every positive target failed the pattern-matched forger with null >= observed share 1.000000. The stronger duplicate-pattern title target `e-me-bal-me-luh-ha` produced no strict candidate. Accepted external anchors remain zero.

Current context-lead matched-negative run: 14 lead families and 52 unique paginated CDLI queries were tested. `szu-i3-li2-su` now has 207 returned artifacts over three pages, 175 query-line hits, and only one Meluhha-bearing query-line artifact, for false-positive rate 0.994286 as a Meluhha detector. `lu2-tukul`, `gurusz`, `nu-banda3`, `ur-{d}lamma`, and `ur-{d}ig-alim` likewise fail as standalone anchors. Nineteen source-side phrases survive only as cuneiform context leads, mostly because they contain `me-luh-ha`; external retesting finds 23 query/site bridge rows, 4 Ur/Susa/Failaka-focus rows, 8 same-site length-pattern matches, and zero object-level bridges. Accepted external anchors remain zero.

Current Ur/Gadd accession bridge audit: `3898.1` is now tied to `U17649` and `3899.1` to `U8685` through local excavation IDs and Gadd/Penn routes. Both are Indus-inscribed external objects with no readable cuneiform/name/title bridge. In the modeled Gadd surface, seven objects are Indus-sequence-only and one (`BM 120573 / U.7683`) is cuneiform-only; zero are micro-bilinguals. Accepted external anchors remain zero.

Current Gulf Type acquisition queue: Laursen Table 1 external `Gulf INDUS` rows `6-27` are now normalized into `gulf_type_indus_external_queue.csv`. The immediate high-pressure rows are Kjaerum cat. `319/279` for Failaka, Amiet `1643` for Susa, Gadd nos. `15/16` for Ur, and Sarzec/Heuzey plate `30.3a-b` for Girsu. This is acquisition infrastructure only.

Current Failaka source acquisition guardrail: CDLI publication `1773730` was cached and parsed as an adjacent Kjaerum/Failaka route. Its four related artifacts cite `168-189`, `379`, `397`, and `399`, not Laursen's target `279/319`; the DAI download route returned anti-bot HTML rather than a PDF. The replacement branch also cached publisher/library access routes through Aarhus University Press, CiNii Books, and Open Library, but none exposes the target catalogue pages digitally. The Failaka rows stay live as source-acquisition targets only.

Current object-level onomastic value attempt: 12 cuneiform-side Meluhha strings were tested against 30 external Mesopotamia/Gulf rows with parseable signs. The only strict mapped-object attempt is `ur gun3-a me-luh-ha` against `3898.1/U17649`, proposing `002=ur;004=gun3;328=a;001=me;803=luh;415=ha`. It is rejected because `U17649` is Indus-only and the cuneiform phrase is from a separate text; target-site shuffled nulls reproduce at least one strict mapped same-site pattern attempt in 0.6857 of iterations.
