# 032-002-861 / 390-004 Branch and 125 Ecology
...
`125` is not supported as a general post-`861` suffix. It occurs `59` times locally; `17` are terminal, `2` are immediate after `861`, and only `1` is immediate after `002-861`. That singleton is H-55.

The stronger new clue is different: `125` appears after `002-390` in `4` rows, including Sktd-1 in the `390-004-002` branch family. Those rows are nonterminal continuations, so the next target is `002-390-125-X`, not isolated `125`.
