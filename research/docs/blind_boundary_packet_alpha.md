...
The blind sheet and the individual blind panels show only neutral IDs — nothing that hints at which object a panel came from. A reader's job is simple: segment the visible marks, record boundaries in both left-to-right and right-to-left descriptions if needed, mark repeated visual clusters across panels, and reject panels that cannot be segmented.
