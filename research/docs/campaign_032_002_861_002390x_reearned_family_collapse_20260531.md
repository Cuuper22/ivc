...
4. `strict_family_core_unmatched`: strict rows preserve branch-tail polarity but still have no strict same-predecessor branch split.

## Decision

The positive branch-tail ecology remains alive, but the two tempting subframe claims are demoted rather than promoted. The adversarial copied/formula-family model still survives because the clean strict layer has polarity without a matched predecessor split, and because `125-632-032` spreads outside the in-frame pair.
