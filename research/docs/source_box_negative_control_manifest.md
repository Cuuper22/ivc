...
The manifest excludes all 25 rows from the `032-002-Y` route list and scans strict Lipi numeric-clean rows for three negative classes:
...
The last line matters. The rarest and most valuable negative class, `220-032` not followed by `002`, exists in metadata but currently has no local image-backed controls. That is a real blocker for a fully matched source-box false-positive rate against the `A-220-032` lane.

## Blind Packet v1

The blind packet contains 20 neutral image files:
