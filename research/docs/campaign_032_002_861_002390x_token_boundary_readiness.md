...
This note is a work order written before the work. The next step in the campaign is to go back to the published photographs and mark where each sign starts and ends — token boxing. Not every object has a good enough image for that. This note sorts the objects into ready and not ready, and, importantly, writes down in advance what result would count as a failure. Committing to the demotion rules before looking is what keeps the test honest.

## Question

Which `002-390-X` witnesses are ready for source-token boundary judgment, and what would demote `125` if the visible source bands do not preserve the catalog window? A "witness" is a database row that attests a sign sequence; a "source band" is the strip of the seal carrying the inscription; the "catalog window" is the run of signs the catalogue transcription claims to be there.

## Result

The next inference unit is a token-boundary packet — a bundle of images and rules assembled for one judgment — not a value assignment.

Strict target rows ready for blind token boxing — "strict" meaning the reading can be checked against an inspectable photograph, "blind" meaning the boxes are drawn without the analyst seeing the claimed reading first: `M-119` and `M-735`.

Strict non-`125` controls ready for the same treatment (a "control" being a row that shares part of the pattern but takes a different branch): `M-70` and `M-71`.

Downweighted public-panel target — the object is visible on a published plate, but that counts for less than a strict row: `Sktd-1`.
...
These are the failure conditions, fixed in advance. If one fires, the named object loses evidential rank:
...
If either target fails source-token boxing, `125` drops back toward formula-family pressure — the rival account in which the matching rows are copies of one stock phrase rather than productive structure. If both fail, `125` is demoted from live source-visible branch evidence to catalog/source-route pressure only.
