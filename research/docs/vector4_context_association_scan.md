...
Both modes ran 100 iterations for each null model:
...
The reported false-positive fields are family-wise. A candidate is compared against the strongest discovered association in each null run, not only against the same pair.
