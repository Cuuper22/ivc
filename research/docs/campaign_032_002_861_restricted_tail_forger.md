...
`603` occurs elsewhere in the corpus. The narrower live question is whether `533-717` is a real fixed-prefix terminal formula after `002-861`, or just another tiny post-hoc tail cell.

## Method

The forger scans strict local rows in two scopes:
...
For each scope it scans all units of length 1, 2, and 3. A candidate must have support at least 2 and every occurrence must be a terminal tail after the fixed prefix:
...
False-positive rates for reproducing the observed max support:
...
Both target rows have source-visible same-line terminal-side material:
...
Exact source-normalized `861/533/717` token boundaries are not accepted. The source packet supports same-line terminal-side material only.

Source-family review rejects exact copy collapse: the two rows differ in source leaf, printed page, full text, length, class, excavation identifiers, depth, boss, dimensions, and immediate pre-`002-861` context. But they remain one narrow Mohenjo-daro no-icon `SEAL:R` source/register-family cell for linguistic weighting.
...
Broad post-hoc prefix searching is dangerous:
...
Accepted as one narrow structural finding:
