...
This note nominates one object for the external-anchor lane, and then explains at length why it is not an anchor.

An external anchor is evidence from outside the Indus corpus that fixes something about it independently — the kind of foothold a Rosetta Stone gives. A lane is a standing workstream in this project. The object is a Mesopotamian cylinder seal whose owner is called an interpreter of the Meluhhan language. It is the best-provenanced candidate we have, and it is still not an anchor.
...
Each numbered line is one independent record agreeing with the others, which is what makes the binding hold:
...
## Fringe claims stay quarantined

Quarantined means recorded, so we know the claim exists, but walled off so it can never be counted as support.

Some non-peer-reviewed sources (e.g. Kalyanaraman) assert the seal carries readable "Indus Script hieroglyphs" encoding a metals-trade message. That is a decipherment-style claim, not accepted scholarship. Cite it only as autopsy of a claim, never as support.
