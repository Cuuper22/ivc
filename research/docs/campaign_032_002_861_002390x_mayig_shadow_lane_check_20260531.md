...
This is a crosswalk/corpus triage guard only. It does not accept a Lipi-to-Mayig sign mapping, source transcription, grammar, value, phonetics, language identity, function, sign meaning, or translation.
...
The local matched-lane gate says the live `032` predecessor split depends on:

- strict/source-visible `M-70 +226-032-002-390-692+`
- blocked/unbound `3335.1 +740-205-032-002-390-590-032+`

The question here was whether the Mayig public-git P namespace, through the local provisional crosswalk, exposes a replacement witness for the blocked `032` lane or any new public-git shadow row that should change acquisition priority.
...
This is the important adversarial result. If we broaden from exact `P145 P122 P086` to just `P122 P086`, the Mayig layer starts mixing local `002-390` with local `002-405`. The collision is not subtle: the provisional reverse edges include `390 -> P086`, but also weak, unaccepted `405 -> P086`, `406 -> P086`, and `407 -> P086` edges.
...
- The broader Mayig `P122 P086` pair is an adversarial collision guard, not a promotion route.
